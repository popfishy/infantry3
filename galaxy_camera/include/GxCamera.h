...
class GxCamera : public rclcpp::Node
{

public:
    explicit GxCamera(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
    ~GxCamera();
...
private:
    ...
    GX_STATUS setSharpness();

    /// Set camera NoiseReduction
    GX_STATUS setNoiseReduction();

    GX_STATUS initCamera();

private:
    static GX_DEV_HANDLE g_hDevice;                    ///< Device handle
    ...
    bool enable_trigger_mode_;                   ///<是否开启外界硬触发模式
    ...
    //ROS2
    std::string camera_name_;                   //节点名称
    static sensor_msgs::msg::Image image_msg_;         //发布图片消息
    static sensor_msgs::msg::CameraInfo camera_info_msg_; //相机信息
    static image_transport::CameraPublisher camera_pub_;   //发布image_msg_以及camera_info_msg_
    std::string frame_id_;                           //相机坐标系
