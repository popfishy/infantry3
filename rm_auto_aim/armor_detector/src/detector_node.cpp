...
{
static int enemy_color = -1;
void gameCallback(const auto_aim_interfaces::msg::GameStatus::SharedPtr game_status_msg)
{
  enemy_color = game_status_msg->enemy_color;
}
...
: Node("armor_detector", options)
{
  ...
  node_ = rclcpp::Node::make_shared("set_enemy_color");
  game_status_sub_ = node_->create_subscription<auto_aim_interfaces::msg::GameStatus>(
    "/game_status", rclcpp::QoS(rclcpp::KeepLast(1)), &gameCallback);
  while (enemy_color == -1) {
    rclcpp::spin_some(node_);
  }
  auto color = enemy_color == 0 ? "RED" : "BLUE";
  RCLCPP_INFO(this->get_logger(), "set enemy color:%s!", color);
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector()
{
  ...
  int min_lightness = declare_parameter("binary_thres", 120, param_desc);

  param_desc.description = "0-RED, 1-BLUE";
  param_desc.integer_range[0].from_value = 0;
  param_desc.integer_range[0].to_value = 1;
  auto detect_color = declare_parameter("detect_color", enemy_color, param_desc);
